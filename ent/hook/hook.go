...
// The WaletFunc type is an adapter to allow the use of ordinary
// function as Walet mutator.
type WaletFunc func(context.Context, *ent.WaletMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f WaletFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.WaletMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.WaletMutation", m)
}
